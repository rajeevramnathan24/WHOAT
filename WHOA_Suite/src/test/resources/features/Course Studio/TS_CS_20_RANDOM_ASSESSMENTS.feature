Feature:CS020: Verify new Courses can be created with Randomized Assessments
  
	Given As an Admin user
  When logged in to course Studio
  Then user should be able to create new courses with Randomized Assessments
    
  @RandomAssessment @Studioregression
  Scenario Outline: To verify that Registered User is able to create new courses with Randomized Assessments
    ...
    Examples: 
     ...
     |1 | whoatestautomation@yopmail.com  | Demo@123         | RDMA       | RandomA |
     # Note: Do not change the sequence of component types     
      
      # sample component type names: HTML-Text, Problem-Dropdown, Video, Zoom-Int, Discussion, RandomA
